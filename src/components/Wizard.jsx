      ...
      <li onClick={() => setSelectedItem(item.id)} className="">
        <div className="flex flex-row items-center justify-between cursor-pointer">
          <div className="flex flex-row items-center">
            <div
              className={`flex justify-center items-center w-6 h-6 rounded-md ml-6 mr-2 ${
                sel ? "bg-blue" : " bg-text bg-opacity-10 "
              }`}
            >
              <p
                className={`font-roboto text-[12px]] font-bold ${
                  sel ? "text-white" : "text-text"
                }`}
              >
                {item.num}
              </p>
          ...
          {sel && (
            <img
              src={downIcon}
              alt="downIcon"
              className="justify-self-end mr-6"
              onClick={() => setSelectedItem(0)}
            />
          )}
        </div>
        {!item.last && !sel && (
          <div className="w-[2px] h-2 ml-[36px] my-2 bg-text bg-opacity-10" />
        )}
        {sel && (
          <WizardSubItem
            item={item}
        ...
        <div>
          <ul className="gap-[13px]">
            {item.childrens.map((child) => (
              ...
              >
                <p
                  className={`font-roboto text-[12px] ml-[19px] mr-[24px] cursor-pointer
                                ${
                                  selectedSubItemId === child.id
                                    ? "text-blue"
